...
def predict_val(model, device,tokenizer,decoder_tok=None):
    ...
    with open("asap_data/val.txt", "r", encoding="utf-8") as f:
        fs = list(f.readlines())
    train_data = []
    count = 0
    total = 0
...
def predict_test(model, device,tokenizer,decoder_tok=None):
    ...
    print("start test")
    model.config.use_cache = False
    with open("asap_data/test.txt", "r", encoding="utf-8") as f:
        fs = list(f.readlines())
    train_data = []
    count = 0
    total = 0
...
def evaluate_val_accuracy_acd(model, val_file_path):
    ...
    for line in val_data:
        s, t, p = line.strip().split("\001")
        x = f"The {t.lower()} category is [MASK] " + s
        label = p
        val_data_formatted.append([x, label])
...
def predict_val_acd(model, device, tokenizer, decoder_tok=None):
    ...
    model.eval()
    model.config.use_cache = False
    with open("asap_acd_data/val.txt", "r", encoding="utf-8") as f:
        fs = list(f.readlines())
    count = 0
    total = 0
    for line in tqdm(fs):
...
def predict_test_acd(model, device, tokenizer, decoder_tok=None):
    ...
    print("start test")
    model.config.use_cache = False
    with open("asap_acd_data/test.txt", "r", encoding="utf-8") as f:
        fs = list(f.readlines())
    train_data = []
    count = 0
    total = 0
