...
from test_MAMS import predict_val_acd, predict_test_acd
...
MODEL_CLASSES = {
    ...
    "t5": (T5Config, T5ForConditionalGeneration, BertTokenizer),#T5Model
}
...
class Seq2SeqModel:
    def __init__(
        ...
        self.args = load_model_args(encoder_decoder_name)
        self.args.cache_dir = kwargs.get("cache_dir", self.args.cache_dir)
        ...
        if encoder_decoder_type in ["bart", "marian", "t5"]:
            ...
            elif encoder_decoder_type in ["marian", "t5"]:
            ...
            raise ValueError("Invalid encoder-decoder type")
    ...
    def train_model(
            self, train_data, best_accuracy=0, output_dir=None, show_running_loss=True, args=None, eval_data=None,
            verbose=True, **kwargs,
    ):
        """
    ...
    def train(
            self, train_dataset, output_dir, best_accuracy, show_running_loss=True, eval_data=None, verbose=True,
            **kwargs,
    ):
        """
        ...
        for current_epoch in train_iterator:
            ...
            accuracy = predict_val_acd(model, self.device, self.encoder_tokenizer)
            print('batch: ' + str(args.train_batch_size) + ' accumulation_steps: ' + str(
                args.gradient_accumulation_steps) + \
                  ' lr: ' + str(args.learning_rate) + ' epochs: ' + str(args.num_train_epochs) + ' epoch: ' + str(
                epoch_number))
            if accuracy > best_accuracy:
            # if True:
                print(f"best_accuracy:{accuracy}")
                best_accuracy = accuracy
                self._save_model(self.args.output_dir, optimizer, scheduler, model=model)
                print('---test dataset----')
                test_acc = predict_test_acd(model, self.device, self.encoder_tokenizer)
                with open(f'./asap_acd_{self.args.model_type}_best_accuracy.txt', 'a+') as f0:
                    f0.writelines('batch: ' + str(args.train_batch_size) + ' accumulation_steps: ' + str(
                        args.gradient_accumulation_steps) + \
                                  ' lr: ' + str(args.learning_rate) + ' epochs: ' + str(
                        args.num_train_epochs) + ' epoch: ' + str(epoch_number) + ' val_accuracy: ' + str(
                        best_accuracy) + \
    ...
    def predict(self, to_predict):
        ...
        for batch in [
            ...
            if self.args.model_type in ["marian", "t5"]:
        ...
        def _decode(output_ids):
            self.decoder_tokenizer.decode(output_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)
        ...
        if self.args.use_multiprocessed_decoding:
            ...
            with Pool(self.args.process_count) as p:
                outputs = list(
                    tqdm(
                        p.imap(_decode, all_outputs, chunksize=self.args.multiprocessing_chunksize),
    ...
    def _get_inputs_dict(self, batch):
        device = self.device
        if self.args.model_type in ["bart", "marian", "t5"]:
