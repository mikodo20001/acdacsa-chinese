...
def classify_predict_logits(model_simple, fs):
    ...
    for line in fs:
        ...
        x = "The sentiment polarity of " + t.lower() + " is [MASK] " + s
        label = p
...
def test_ensemble(bert_path, bart_path1, bart_path2=None, weights=[1, 1, 1]):
    ...
    model1 = ClassificationModel("roberta", bert_path, num_labels=3, args=model_args)
    model2 = Seq2SeqModel(encoder_decoder_type="bart", encoder_decoder_name=bart_path2, args=model_args, )
    # model3 = Seq2SeqModel(encoder_decoder_type="marian",encoder_decoder_name=bart_path1,args=model_args,)
    ...
    def test(fp):
        ...
        print(f"acc:{accuracy_score(pred, label)}")
        print("=========end===========")
        output_dir = "ensmble_out"
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)
        file_names = output_dir + "/R%.2d_B%.2d_output.txt" % (10 * weights[0], 10 * weights[1])
    ...
    # test("./asap_data/val.txt")
    test("./asap_data/test.txt")
...
for R in np.arange(0, 1.1, 0.1):
    ...
    for B in np.arange(0, 1.1, 0.1):
        ...
        if t5_weight >= 0:
            ...
            test_ensemble("./bert", "./t5", "./bart", weights)
            epoch += 1
